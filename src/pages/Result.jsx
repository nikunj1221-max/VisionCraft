...
const Result = () => {
  ...
  return (
    ...
    <div className='flex w-full max-w-xl bg-neutral-500 text--white text-sm p-0.5 mt-10 rounded-full  '>
      ...
      <button type='submit' className='bg-zinc-900 sm:px-16 py-3 rounded-full text-white '> Generate</button>
    </div>}
    {isImageLoaded &&
    <div className='flex flex-wrap gap-2 justify-center text-white text-sm p-0.5 mt-10 rounded-full' >
      <p onClick={()=>{setIsImageLoaded(false)}}  className='bg-transparent border border-zinc-900 text-black px-8 py-3 rounded-full cursor-pointer'>  Generate Another </p>
        {/* <a href={image}  download={image} className='bg-zinc-900  px-10 py-3 rounded-full cursor-pointer'
    > Download </a> */}
    <button onClick={handleDownload}  className='bg-zinc-900  px-10 py-3 rounded-full cursor-pointer'>
  Download
</button>
