...
const Testimonials = () => {
  return (
    ...
    viewport={{ once: true }}
    className='flex flex-col items-center justify-center my-20 p-12 md:px-28'>
            ...
            {testimonialsData.map((testimonial , index)=>(
                    ...
                    <div key={index} className='bg-white/20 p-12 rounded-lg shadow-md order w-80 m-auto cursor-pointer hover:scale[1.02] transition-all '>
                        <div className='flex flex-col items-center '>
                            ...
                            <p className='text-gray-500mb-4'>{testimonial.role}</p>
                            <div className='flex mb-4 '> 
            {Array(testimonial.stars).fill().map((item,index)=>(
                <img key={index} src={assets.rating_star} alt="" />
            ))}
                            </div>
                            <p className='text-center text-sm tet-gray-600'>{testimonial.text}</p>
                        </div>
                    </div>
            ))}
