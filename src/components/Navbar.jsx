...
export const Navbar = () => {
  ...
  return (
    <div className='flex items-center justify-between py-4'> 
       <Link to='/'>
       <img src={assets.logo} alt="" className=' w-40 sm:w-32 lg::w-40 '/>
       </Link>   
       <div>
        {user ? 
          <div className=' flex items-center gap-2 sm:gap-3  '> 
             <button onClick={()=>navigate('/buycredit')} className='flex items-center gap-2 bg-blue-100 px-4 sm:px-6 py-1.5 sm:py-3 rounded-full hover:scale-105 transition-all duration-700'>
                <img src={assets.credit_star} className='w-5' alt="" />3
        ...
        : 
           <div className='flex items-center gap-2 sm:gap-5 '>
             ...
             <button onClick={()=>setShowLogin(true)} className='bg-zinc-800 text-white px-7 py-2 text-sm rounded-full  '>Login </button>
           </div>
         }
